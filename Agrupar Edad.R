...
results_casos_edad <- data.frame(matrix(ncol=4, nrow=0))                   # Crear el data frame de los resultados y nombrar las columnas
colnames(results_casos_edad) <- c("Desease","Año","Resultado","Mensaje")
...
for (i in 1:nrow(des_ano_no_error)) {                                                                            
  ...
  if (probar_agrupar_edad$res == "Error") {
    ...
    casos_edad <-  probar_agrupar_edad$data
    new_row <- list(desease,year, probar_agrupar_edad$res, probar_agrupar_edad$msg)          # Crear la nueva fila para los resultados con el desase, año y resultado
    results_casos_edad[nrow(results_casos_edad) + 1,] <- new_row                    # Agregar la nueva fila a los resultados
  }
  
}
#df <- apply(results_casos_edad,2,as.character)
#write.csv2(df,"Results probaragruparedad.CSV",row.names = FALSE, na='')
