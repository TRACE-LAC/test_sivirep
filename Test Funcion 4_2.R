...
results_fechanotif <- data.frame(matrix(ncol=4, nrow=0))                   # Crear el data frame de los resultados y nombrar las columnas
colnames(results_fechanotif) <- c("Desease","Año","Resultado","Mensaje")
...
for (i in 1:nrow(des_ano_no_error)) {                                                                            
  ...
  if (fechanotifica$res == "Error") {
    ...
    casos_fecha_notificacion <- fechanotifica$data
    new_row <- list(desease,year,fechanotifica$res,fechanotifica$msg)          # Crear la nueva fila para los resultados con el desase, año y resultado
    results_fechanotif[nrow(results_fechanotif) + 1,] <- new_row                    # Agregar la nueva fila a los resultados
  }
