...
try_geofilter <- function(data_event_limp, dpt) {
  tryCatch(
    ...
    error = function(e){
      err_msg = paste("Error geo_filtro ",dpt,": ",e)                                   # Mensaje de error
      print(err_msg)
      list(data= "", res="Error",msg=e)                                                    # Devolver como resultado "Error" y el mensaje de error
    },
    warning = function(w){
      wrn_msg = paste("Warning geo_filtro ",dpt,": ",w)                                 # Mensaje de warning
      print(wrn_msg)
...
# 1. Funcion para correr agrupar_mpio y que diga si hay error
...
results_dist_esp_dept <- data.frame(matrix(ncol=5, nrow=0))                   # Crear el data frame de los resultados y nombrar las columnas
colnames(results_dist_esp_dept) <- c("Desease","Año","Departamento", "Resultado","Mensaje")

#Inicio loop de todas las fila des_ano_no_error
desease <- des_ano_no_error$Desease[1]
year <- des_ano_no_error$Año[1]

for (i in 2176:nrow(des_ano_no_error)) {                                                                            
  newdesease <- des_ano_no_error$Desease[i]
  newyear <- des_ano_no_error$Año[i]
  
  #No importar si desases y year es el mismo
  if (newdesease!=desease | newyear!=year) { 
    year=newyear
    desease =newdesease
        # Importar datos
  ...
  #Probar geofiltro
  dpto <- des_ano_no_error$Dpto[i]
  geo <- try_geofilter(data_event = data_event_limp, dpto)
  if (geo$res == "Error"){
    result_msg <- geo$msg
    new_row <- list(desease,year,dpto,"Error",result_msg)    # Crear la nueva fila para los resultados con el desase. año y resultado
    results_dist_esp_dept[nrow(results_dist_esp_dept) + 1,] <- new_row                    # Agregar la nueva fila a los resultados
    next                                                      # Ignorar el resto del código y saltar al siguiente año
  }
  ...
  if (probar_agrupar_mpio$res == "Error") {
    result_msg <-  probar_agrupar_mpio$msg
    new_row <- list(desease,year,dpto, "Error",result_msg)    # Crear la nueva fila para los resultados con el desase. año y resultado
    results_dist_esp_dept[nrow(results_dist_esp_dept) + 1,] <- new_row                    # Agregar la nueva fila a los resultados
    next
    ...
    new_row <- list(desease,year, dpto, probar_agrupar_mpio$res, probar_agrupar_mpio$msg)          # Crear la nueva fila para los resultados con el desase, año y resultado
    results_dist_esp_dept[nrow(results_dist_esp_dept) + 1,] <- new_row                    # Agregar la nueva fila a los resultados
  }
  
}
#df <- apply(results_dist_esp_dept,2,as.character)
#write.csv2(df,"Results agrupar_mpio2.CSV",row.names = FALSE, na='')
