...
results_casos_edad_sex <- data.frame(matrix(ncol=4, nrow=0))                   # Crear el data frame de los resultados y nombrar las columnas
colnames(results_casos_edad_sex) <- c("Desease","Año","Resultado","Mensaje")
...
for (i in 1:nrow(des_ano_no_error)) {                                                                            
  ...
  if (probar_agrupar_edad_sex$res == "Error") {
    ...
    casos_edad_sex <-  probar_agrupar_edad_sex$data
    new_row <- list(desease,year, probar_agrupar_edad_sex$res, probar_agrupar_edad_sex$msg)          # Crear la nueva fila para los resultados con el desase, año y resultado
    results_casos_edad_sex[nrow(results_casos_edad_sex) + 1,] <- new_row                    # Agregar la nueva fila a los resultados
  }
