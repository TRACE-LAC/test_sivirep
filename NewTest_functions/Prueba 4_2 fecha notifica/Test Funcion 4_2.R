...
# 1. Funcion para correr agrupar_fecha_notifica y que diga si hay error
try_fechanotifica <- function (data_event_limp, desease, year) {
  tryCatch(
    {
    ...
    error = function(e){
      err_msg = paste("Error agrupar_fecha_notifica",desease,year,": ",e)                         # Mensaje de error
      print(err_msg)
      list(data="",res="Error",msg=e)                                           # Devolver como resultado "Error" y el mensaje de error
    },
    warning = function(w){
      wrn_msg = paste("Warning agrupar_fecha_notifica",desease,year,": ",w)                       # Mensaje de warning
      print(wrn_msg)
      list(data=casos_fecha_notificacion,res="Warning",msg=w)                                 # Devolver como resultado los datos,  "warning" y el mensaje de warning
    }
  )
}


results_fechanotif <- data.frame(matrix(ncol=5, nrow=0))                   # Crear el data frame de los resultados y nombrar las columnas
colnames(results_fechanotif) <- c("Desease","Año","Resultado", "Funcion donde se obtuve el resultado","Mensaje")
...
for (i in 1:n_deseases) {                                       # Iterar i desde 1 hasta la cantidad de enfermedades
  ...
  for (year in years) {                                         # Iterar sobre todos los años del desease
    ...
    if (data_event_result$res == "Error"){
      ...
      results_fechanotif[nrow(results_fechanotif) + 1,] <- new_row                    # Agregar la nueva fila a los resultados
      next    
    }
    ...
    #Probar la función
    fechanotifica <- try_fechanotifica(data_event_limp, desease, year)
    result <- fechanotifica$res
    result_msg <- fechanotifica$msg
    new_row <- list(desease,year,result,"agrupar_fecha_notifica",result_msg)          # Crear la nueva fila para los resultados con el desase, año y resultado
    results_fechanotif[nrow(results_fechanotif) + 1,] <- new_row                    # Agregar la nueva fila a los resultados
    
  }
}
#write_xlsx(results_fechanotif, "Results agrupar_fecha_notifica.xlsx")

df <- apply(results_fechanotif,2,as.character)
write.csv2(df,"Results agrupar_fecha_notifica.csv",row.names = FALSE, na='')
