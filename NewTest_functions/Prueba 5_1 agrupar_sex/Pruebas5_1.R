...
results_casos_sex <- data.frame(matrix(ncol=5, nrow=0))                   # Crear el data frame de los resultados y nombrar las columnas
colnames(results_casos_sex) <- c("Desease","Año","Resultado", "Funcion donde se obtuve el resultado","Mensaje")
...
for (i in 1:n_deseases) {                                       # Iterar i desde 1 hasta la cantidad de enfermedades
  ...
  for (year in years) {                                         # Iterar sobre todos los años del desease
    ...
    result <- probarsex$res
    result_msg <- probarsex$msg
    new_row <- list(desease,year,result,"agrupar_sex",result_msg)          # Crear la nueva fila para los resultados con el desase, año y resultado
    results_casos_sex[nrow(results_casos_sex) + 1,] <- new_row                    # Agregar la nueva fila a los resultados
    
  }
}
#write_xlsx(results_casos_sex, "Results agrupar_sex.xlsx")

df <- apply(results_casos_sex,2,as.character)
write.csv2(df,"Results agrupar_sex.csv",row.names = FALSE, na='')
