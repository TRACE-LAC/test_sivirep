...
try_geofilter <- function(data_event_limp, desease, year, dpt) {
  tryCatch(
    ...
    warning = function(w){
      ...
      print(wrn_msg)
      list(data=data_event_filtrada, res="Warning",msg=wrn_msg)                                                 # Devolver como resultado "warning" y el mensaje de warning
    }
  )
}
...
try_agrupar_mpio<- function(data_event_filtrada, desease, year, dpto) {
  tryCatch(
    ...
    warning = function(w){
      ...
      print(wrn_msg)
      list(data=dist_esp_dept,res="Warning",msg=wrn_msg)                                 # Devolver como resultado los datos,  "warning" y el mensaje de warning
    }
  )
}
...
write_xlsx(results_dist_esp_dept, "Results agrupar_mpio.xlsx",)
