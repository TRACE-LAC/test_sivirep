...
# 1. Funcion para correr agrupar_edad y que diga si hay error
try_agrupar_edad<- function (data_event_limp, desease, year) {
  tryCatch(
    {
      casos_edad <-  agrupar_edad(data_event = data_event_limp,interval_edad = 10)
      result_dim <- dim(casos_edad)                                          # Guardar las dimensiones (filas, columnas) del data frame resultante
      succ_msg <- paste("Success",desease,year,":",result_dim[1]," rows,",result_dim[2]," columns")
      print(succ_msg)
      list(data=casos_edad,res="Success",msg=succ_msg)                     # Devolver como resultado los datos, "Success" y un mensaje las dimensiones del data frame
    },
    error = function(e){
      err_msg = paste("Error agrupar_edad",desease,year,": ",e)                         # Mensaje de error
      print(err_msg)
      list(data="",res="Error",msg=e)                                           # Devolver como resultado "Error" y el mensaje de error
    },
    warning = function(w){
      wrn_msg = paste("Warning agrupar_edad",desease,year,": ",w)                       # Mensaje de warning
      print(wrn_msg)
      list(data=casos_edad,res="Warning",msg=w)                                 # Devolver como resultado los datos,  "warning" y el mensaje de warning
    }
  )
}


results_casos_edad <- data.frame(matrix(ncol=5, nrow=0))                   # Crear el data frame de los resultados y nombrar las columnas
colnames(results_casos_edad) <- c("Desease","Año","Resultado", "Funcion donde se obtuve el resultado","Mensaje")
...
for (i in 1:n_deseases) {                                       # Iterar i desde 1 hasta la cantidad de enfermedades
  ...
  for (year in years) {                                         # Iterar sobre todos los años del desease
    ...
    if (data_event_result$res == "Error"){
      ...
      results_casos_edad[nrow(results_casos_edad) + 1,] <- new_row                    # Agregar la nueva fila a los resultados
      next    
    }
    ...
    #Seguir si hay error 
    probar_agrupar_edad <- try_agrupar_edad(data_event_limp, desease, year)
    result <- probar_agrupar_edad$res
    result_msg <-  probar_agrupar_edad$msg
    new_row <- list(desease,year, result, "agrupar_edad", result_msg)          # Crear la nueva fila para los resultados con el desase, año y resultado
    results_casos_edad[nrow(results_casos_edad) + 1,] <- new_row                    # Agregar la nueva fila a los resultados
    
  }
}
#write_xlsx(results_casos_edad, "Results agrupar_edad.xlsx")

df <- apply(results_casos_edad,2,as.character)
write.csv2(df,"Results agrupar_edad.csv",row.names = FALSE, na='')
